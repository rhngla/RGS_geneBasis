...
df <- read_feather("/Users/fruity/Local/datasets/RGS/m1_10x_raw.feather")
cols = colnames(df)
genes <- cols[2:10001]
samples <- df$sample
...
for (n_genes in c(256)) {
    fname <-
        paste0("/Users/fruity/Local/datasets/RGS/m1_bin_gene_list_",
               n_genes,
               ".feather")
    startt <- Sys.time()
...
write_feather(time_df, "/Users/fruity/Local/datasets/RGS/m1_bin_elapsed_time.feather")
...
df <- read.csv("/Users/fruity/Local/datasets/RGS/patchseq_lognorm.csv", header = TRUE, sep = ",")
genes = colnames(df)
samples <- 1:dim(df)[1]
...
for (n_genes in c(256)) {
    fname <-
        paste0("/Users/fruity/Local/datasets/RGS/patchseq_lognorm_gene_list_",
               n_genes,
               ".feather")
    startt <- Sys.time()
...
write_feather(time_df, "/Users/fruity/Local/datasets/RGS/patchseq_lognorm_elapsed_time.feather")